...
$newURL = str_replace(DIRECTORY_SEPARATOR . 'wp-content' . DIRECTORY_SEPARATOR . 'plugins' . DIRECTORY_SEPARATOR . 'wp-admin-icons', '', $oldURL);
...
//checks if the form has been submitted
 if(isset($_POST['Submit'])) 
 {
		...
		if (($extension != "png")) 
	 	...
	 	{ 		
			...
			if (!$errors)
			{
				...
				$newname = "../icons/uploads/".$file_name;
				//we verify if the image has been uploaded, and print error instead
				$copied = copy($_FILES['icon']['tmp_name'], $newname);
				if (!$copied) 
				{
	...
	{
		//$tempURL = bloginfo('url');
		$url =  '../../../../wp-admin/options-general.php?page=wp-admin-icons.php/wp-admin-icons-init.php';
